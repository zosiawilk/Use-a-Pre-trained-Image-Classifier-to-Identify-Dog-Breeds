...
def classify_images(images_dir, results_dic, model):
    ...
    for key, value in results_dic.items():
        """Setting the string variable model_label to be the string that's 
        returned from using the classifier function"""         
        model_label = classifier(os.path.join(images_dir, key), model)
                             
        """converting all characters within model_label to                          lowercase letters and removing whitespace characters from the                ends"""
